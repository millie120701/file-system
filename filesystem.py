...
class FileSystemObject:
    ...
    def update_last_modified(self, modified_time=None):
        ...
        if self.parent_folder is not None:
            # some folders may share the same name
            sql_update = "UPDATE Folders SET last_modified = %s WHERE filepath = %s"
    ...
    def rename(self, new_name):
        ...
        self.name = new_name
        # -> performance gain opportunity: instead of re-calculating the whole string, modify old_path (with .split())
        ...
        if type(self) == File:
            ...
            # new name, new path, old path
            val_file_change_update_file = (self.name, new_path, old_path)
            mycursor.execute(sql_file_change_update_file,
                             val_file_change_update_file)
            connection.commit()
            ...
            mycursor.execute(sql_folder_change_update_folder,
                             val_folder_change_update_folder)
            connection.commit()
            ...
            val_folder_change_update_path_of_folders = (
                old_path, new_path, old_path + '%')
            mycursor.execute(sql_folder_change_update_path_of_folders,
                             val_folder_change_update_path_of_folders)
            connection.commit()
            ...
            val_folder_change_update_path_of_files = (
                old_path, new_path, old_path + '%')
            mycursor.execute(sql_folder_change_update_path_of_files,
                             val_folder_change_update_path_of_files)
            connection.commit()
...
class Folder(FileSystemObject):
    ...
    def add_folder(self, name):
        ...
        val = (new_folder.name, new_folder.last_modified,
               new_folder.get_full_path())
    ...
    def get_size(self):
        total_size = 0
        for item in self.content:
            # iterate across files and folders and call their common get_size method, which returns an int
    ...
    # e.g. stuff.return_object("more_stuff/things/notes.txt")
    ...
    def update_folder_size(self):
        # some folders may share the same name
        sql_update = "UPDATE Folders SET size = %s WHERE filepath = %s"
...
class File(FileSystemObject):
    ...
    def modify(self, content):
        ...
        val = (self.content, self.get_size(),
               last_modified, self.get_full_path())
        mycursor.execute(sql_update, val)
        connection.commit()
